...
2. **Confirmation (requires connect):** to know we have the same device, we must connect, run `verifyDevice`, and compare the returned `fingerprint` against the cache's `fingerprint`. The cache stores the hashed canonical fingerprint, and Stage 1's single-producer invariant means `verifyDevice` returns it directly — never re-derive identity from raw `serial+model` outside `verifyDevice`. If they match, success; if not, this is a different device on the same address (port reused) — disconnect and fall through.

The cache stores both `mdnsServiceName` (cheap match) and `fingerprint` (authoritative match). No-args `connect` tries the cache's `lastKnownAddress` for any cached fingerprint first, confirming via `verifyDevice` (the returned `mdnsServiceName` from the cache row, if any, is just a hint that helps prune scan results). On miss or fingerprint mismatch, fall through to a full mDNS scan and confirm-by-fingerprint. (No multi-address brute-force — see "Single cached fingerprint" above.)
