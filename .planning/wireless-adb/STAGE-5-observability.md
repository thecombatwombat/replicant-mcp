...
- **In-server event log** of connect / disconnect / verify-fail / supervisor-reconnect events:
  - Each event: `{ timestamp, event, deviceFingerprint?, address?, reason?, recoveryApplied?, durationMs }`. `deviceFingerprint` is the canonical fingerprint string (`CONTEXT.md`) — already hashed, safe to return to the agent. Raw `serial`/`model` are never event fields.
  - Bounded ring buffer (e.g., last 500 events, configurable). No on-disk persistence in Stage 5 — that can come later if useful.
  - **Event coalescing.** Repeated events of the same coalescing key within a short window (e.g., 10 seconds) collapse into one entry with a `count` field, so a flapping device cannot fill the buffer in minutes. The coalescing key is `(event, identity, normalizedReason)` where `identity` is `deviceFingerprint` if known, otherwise `address`. Including the reason prevents collapsing genuinely different failures into one bucket. **Reason normalization** (applied only for the coalescing key — the event's stored `reason` field keeps the original): lowercase, trim whitespace, strip trailing punctuation, collapse internal whitespace. This means `"Connection refused"` and `"connection refused."` coalesce; `"connection refused"` and `"device offline"` do not. The fallback to `address` matters in early-stage incremental Stage 1 deployments where fingerprint may not yet be populated; without it, two unrelated devices with no fingerprint would appear as one. Backoff-step events from the supervisor are exempt — those are inherently rate-limited by the backoff itself.
