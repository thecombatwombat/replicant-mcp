...
Implementation: (1) vitest.config.ts with thresholds, (2) CI skips coverage for doc-only changes (git diff check on src/), (3) CLAUDE.md updated with testing standards, (4) systematic improvement plan for 8 low-coverage modules, (5) emulator tests run on all PRs except docs/chore/trivial branches — expense justified for quality gate, but skipped when code isn't touched.
...
## [2026-02-08] Git Pre-Push Hook for Master Branch Protection
Tags: workflow, enforcement, git, automation
Context: CLAUDE.md rule "no direct pushes to master" existed but wasn't programmatically enforced — Claude agents (and developers) repeatedly pushed to master during work on enforcement mechanisms themselves. Pure documentation-based rules are easily forgotten under time pressure or in the flow of development.
Decision: Implement git pre-push hook (`.githooks/pre-push`) to block direct pushes to master/main branches locally. Hook allows exception for automated release commits matching pattern `chore: release v*`. Warn (but don't block) for non-standard branch names outside `feature/`, `fix/`, `docs/`, `refactor/`, `chore/`, `trivial/` prefixes. SessionStart hook auto-configures hooks path to eliminate manual setup.
Implementation: (1) POSIX sh-compatible pre-push hook with `#!/usr/bin/env sh` shebang for cross-platform compatibility (Ubuntu/Debian use dash). (2) Hook reads refs from stdin, extracts branch names, blocks master/main unless latest commit message starts with `chore: release v[0-9]`. (3) Uses `continue` not `exit 0` after release exception to validate remaining refs (prevents security bypass). (4) `.claude/hooks/SessionStart.sh` auto-runs `git config core.hooksPath .githooks` on session start. (5) `.githooks/README.md` documents manual setup for non-Claude Code users.
Alternatives: GitHub branch protection (rejected: requires repository admin access, doesn't help local-first development), trust-based documentation only (rejected: proven ineffective), bash-specific syntax with `#!/bin/bash` (rejected: incompatible with systems where sh is dash), global git hooks via `git config --global core.hooksPath` (rejected: affects all repos, not project-specific).
Refs: .githooks/pre-push, .githooks/README.md, .claude/hooks/SessionStart.sh, docs/plans/2026-02-08-claude-md-workflow-enforcement.md

## [2026-02-11] Machine-readable tool contracts and contract testing
Tags: testing, validation, schema, architecture
Context: External tools (MCP clients, test frameworks, documentation generators) need machine-readable contracts to validate against replicant-mcp's tool interface. Manual maintenance of separate contract files leads to drift. Need automated validation that tool implementations match their declared schemas.
...
## [2026-05-06] CI-driven MCP Registry publishing with pinned mcp-publisher
Tags: workflow, ci, release, mcp-registry, distribution
Context: The official MCP Registry (`registry.modelcontextprotocol.io`) does not crawl npm — it requires explicit `mcp-publisher publish` calls. Our release flow only ran `npm publish` and created a GitHub Release, so the registry drifted: it sat at v1.6.3 while npm had v1.6.6. The two registry entries that did exist (v1.6.1, v1.6.3) were ad-hoc manual pushes, not a sustainable process. True atomicity is impossible because `npm publish` is irreversible (npm permanently tombstones unpublished versions after 72h), so we get as-atomic-as-feasible by pre-flighting every reversible step before the irreversible one.
Decision: Add MCP Registry publish to `.github/workflows/release.yml` in a single job alongside the npm publish, ordered so pre-flight (install + validate + `login github-oidc`) runs before `npm publish`, and the actual `mcp-publisher publish .mcp/server.json` runs after. By the time we hit npm, schema and auth are already validated. Pin `mcp-publisher` to a specific version (`MCP_PUBLISHER_VERSION` env var, currently 1.7.8) so a CLI breaking change can't surprise a release. Use OIDC auth — no secrets to manage. `.mcp/server.json` is the source of truth; `scripts/release.sh` keeps it in sync with `package.json` and runs a soft local validate when `mcp-publisher` is on PATH.
Alternatives: Float on `mcp-publisher@latest` (rejected: floating dependency in an irreversible flow); two-job split with `needs: release` (rejected: forces a second OIDC login and gives no atomicity benefit); manual `mcp-publisher publish` from a laptop after each release (rejected: that's how we got here); GitHub PAT instead of OIDC (rejected: long-lived secret with no operational benefit).
Refs: .github/workflows/release.yml, scripts/release.sh, .mcp/server.json, https://github.com/modelcontextprotocol/registry/blob/main/docs/reference/cli/commands.md

## [2026-05-15] Internal adb-shell payloads never use shell composition operators
Tags: safety, adb, architecture, internal-callers
Context: THE-105. `visual-snapshot` failed with `COMMAND_BLOCKED: Shell metacharacters are not allowed in shell commands`. Root cause: `UiAutomatorAdapter.getCurrentApp` built shell payloads like `dumpsys activity activities | grep mResumedActivity` and `dumpsys window | grep mCurrentFocus`. The `|` was correctly rejected by our own `validateShellPayload` guard in `src/services/process-runner.ts`. The guard is the right behavior — it stops chained / substitution payloads from sneaking past via user-controllable strings — but internal trusted code was hitting it from the inside.
Decision: Internal callers of `adb.shell` never use shell composition operators (`|`, `&&`, `;`, backticks, `$()`, subshells). They issue a single command and parse the full output in TypeScript. Parsers live in `src/parsers/` so the logic is unit-testable in isolation. The shell safety guard remains the single boundary contract — no internal bypass, no whitelist of "trusted" pipes.
Alternatives: (a) loosen the guard for internally-built strings — rejected, would weaken the boundary and require taint-tracking we don't have; (b) wrap commands in `sh -c "..."` — rejected, the guard also blocks `sh -c` and weakens the security model anyway; (c) keep the pipe and just suppress the guard for `getCurrentApp` — rejected, special-casing the guard is exactly the failure mode the guard exists to prevent.
Refs: src/parsers/dumpsys-current-app.ts, src/adapters/ui-automator.ts (getCurrentApp), src/services/process-runner.ts (validateShellPayload), THE-105

## [2026-05-15] ui-action input verification ships; clipboard-paste deferred (CU-9 / THE-113)
Tags: ui, ui-action, verification, clipboard
Context: THE-113 was two halves. (1) The agent has no way to confirm `input` actually landed in the target field — `input text "hello"` on a non-focused field silently no-ops, or autocomplete may transform what was typed. (2) `adb shell input text` mangles agent text that contains spaces, special characters, or unicode; the canonical fix is to set the system clipboard and press KEYCODE_PASTE. Both halves were on the table.
Decision: Ship the verify half now. Skip the implementation of the clipboard half; design only.
  - Verify: `verify: true` on `input` op. Requires a selector. Captures the field's `text` (or `contentDesc`) via a fresh `find` before and after the input call. Returns `{ verified, containsRequested, changed, inputBefore, inputAfter }`. `verified = containsRequested || changed` — the looser `changed` signal catches autocomplete/case-mutation. Defaults to false (opt-in cost: 2 extra dumps). Internal finds snapshot/restore `lastFindResults` + `lastFindFingerprints` to avoid clobbering whatever the agent had in flight.
  - Clipboard-paste DESIGN (deferred): introduce `AdbAdapter.pasteText(deviceId, text)` that (a) sets the device clipboard via `cmd clipboard set-primary-clip` (Android 12+) — or, as a separate sub-design, broadcasts to a Clipper-style helper on older devices — and (b) sends `input keyevent KEYCODE_PASTE`. The first step's argv places the agent text in its own arg, which the new per-arg shell-payload guard (CU-2) permits even with embedded `&` / `|`. Deferred because: device-capability detection (Android version, focused-field clipboard permission, IME quirks) is its own scope that doesn't share much code with the verify half. The CU-2 work already unblocked the path — clipboard-paste can land as a follow-up without re-touching the guard.
Alternatives: (verify) Compare focused-field via accessibility tree's `accessibilityFocused` flag instead of re-running the same selector — rejected, focus isn't always set on the just-typed-into field, and the selector path already works on the same node. (clipboard) Ship clipboard-paste alongside verify in this PR — rejected, scope creep on an already-substantial PR; the design is captured here so the follow-up can pick it up without re-discovery.
Refs: src/tools/ui-action.ts (handleInput + readSelectorText), src/tools/ui-action-scroll.ts (extracted to keep ui-action.ts under file cap), THE-113

## [2026-05-15] Fingerprint-based stale-element check on elementIndex (CU-8 / THE-112)
Tags: ui, ui-action, safety, cross-call-state
Context: THE-112. `ui-query find` cached match results in `context.lastFindResults`; `ui-action` consumed them via `elementIndex`. Between the two calls, the screen can change — a list scrolls, a dialog dismisses, a snackbar appears. The agent then taps the (centerX, centerY) of what used to be the target, but is now some other element. The ticket offered three paths: (a) document the lifetime and detect staleness at consume time; (b) introduce persistent session-scoped element IDs and a global resolver; (c) decline and document the agent-side workaround. Option (a) catches the real risk (screen moved) without adding a session-scoped ID surface that every tool would have to plumb through.
Decision: Option (a). At find time, capture a per-match content fingerprint = `text|resourceId|className|bounds`. Store as `context.lastFindFingerprints` parallel to `lastFindResults`. At consume time in `ui-action`'s `elementIndex` branch, re-dump the tree, locate the node currently at the cached (centerX, centerY), recompute its fingerprint, and reject with the new `STALE_ELEMENT_INDEX` error code if it differs (or if no node is there). OCR/grid matches get an empty fingerprint and skip the check — they have no stable identity across calls. Empty `lastFindFingerprints` also skips the check (backward compat for any caller pre-populating the cache).
Alternatives: (b) Persistent element IDs — rejected, introduces a session-scoped ID surface used by exactly one cross-tool path; the fingerprint check gives equivalent safety without the API surface. (c) Decline — rejected, the risk is real (the orchestrator brief cites it explicitly). Stricter geometric match (compare bounds rather than center) — rejected, container resizes within the same frame would false-positive; bounds are already part of the fingerprint so any real shift is caught. Hashing the fingerprint — rejected, the strings are short enough that a literal concatenation is cheaper than a hash and easier to inspect in error context.
Refs: src/types/errors.ts (STALE_ELEMENT_INDEX), src/tools/util-fingerprint.ts, src/tools/ui-find.ts, src/tools/ui-action.ts (assertElementStillFresh), THE-112

## [2026-05-15] Per-arg shell-payload guard + typed-intent entrypoint (CU-2 / THE-106)
Tags: safety, adb, shell-guard, intents
Context: THE-106. The `adb-shell` safety guard joined all args after `shell` with spaces and ran a single regex against the result. That caught real chains (`ls; rm`) but also rejected any single arg whose data contained `&`, `|`, `(`, `$`, etc. — including ordinary URLs like `https://example.com/?foo=bar&baz=qux`. The agent could not fire an intent containing a query-string URL, even via argv: the guard saw the joined string and bailed. The COMMAND_BLOCKED was over-broad — execa never hands the joined string to a shell.
Decision: TIGHTEN the guard, don't loosen it. Each payload arg is now scanned individually against a documented set of composition patterns (SHELL_COMPOSITION_PATTERNS). The matchers catch: backticks, `$(...)`, `${VAR}`, `$IDENT`, `(...)` subshells, leading chain operators, `;` anywhere, `&&`/`||` anywhere, and `|` with surrounding whitespace. Embedded `&` and glued `|` (no whitespace) flow through because URLs/regexes/data legitimately use them inside a single token. Additionally, a typed `AdbAdapter.startIntent` method builds `am start -W` as argv, validating action regex, length caps, null-byte rejection, package/component shape, and extras key/value before any argv is constructed. The new `adb-app start-intent` op exposes this.
Alternatives: (a) introduce a `trustedArgv` bypass on `runAdb` — rejected, that's exactly the "internal bypass" pattern the THE-105 ADR forbade; (b) keep the joined-string guard and document the limitation — rejected, makes intents un-firable; (c) URL-encode `&` before sending — rejected, Android's `am start -d` doesn't decode it back the way the agent expects.
Refs: src/services/process-runner.ts (validateArgForShellComposition, SHELL_COMPOSITION_PATTERNS), src/adapters/adb.ts (startIntent), src/tools/adb-app.ts (start-intent op), THE-106

## [2026-05-15] Scroll direction is gesture-named (not content-named); docs-only fix, no alias
Tags: ui, ergonomics, docs, ui-action
Context: THE-110. Agent feedback flagged that `direction: "down"` is ambiguous — could mean "swipe down" or "content moves down." The implementation has always been gesture-based (`"down"` = swipe up = content scrolls down). Two paths were on the table: (a) docs-only — keep `up/down/left/right`, make the tool description explicit; (b) docs + alias — also accept `gesture-up/gesture-down` (or similar) values. The existing values are stable and widely used; introducing aliases doubles the surface and creates two ways to do one thing.
Decision: Docs-only. The schema describe text on `direction` and the tool description both now state the gesture semantics explicitly. No new alias values; no breaking change. If agents still misread it after this, revisit with concrete evidence.
Alternatives: Add `gesture-up/gesture-down` aliases (rejected — schema surface bloat, no evidence the doc fix is insufficient); rename `down` → `gesture-down` outright (rejected — breaking change to a settled API for a clarity issue).
Refs: src/tools/ui-action.ts (direction describe + tool description), THE-110
