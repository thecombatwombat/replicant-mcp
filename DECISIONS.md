...
## [2026-05-08] Remote mode via mcp-proxy with Tailscale-first defaults
Tags: architecture, transport, remote, deployment
Context: replicant-mcp was stdio-only — the MCP client had to launch it as a subprocess on the same machine the phone is plugged into. We needed a way for an agent on machine A to drive a phone connected to machine B. The MCP ecosystem already has [`sparfenyuk/mcp-proxy`](https://github.com/sparfenyuk/mcp-proxy) that bridges stdio servers to HTTP/SSE; reimplementing transport inside replicant-mcp would duplicate it and pull a non-trivial protocol surface (auth, sessions, lifecycle) into a single-author project.
Decision: Add a `replicant-mcp serve --http` subcommand that bundles the wiring but not the proxy itself. On startup the subcommand: (1) checks `uv` is on PATH and uses `uvx mcp-proxy …` to fetch the proxy on first run; (2) auto-detects the Tailscale interface via `tailscale ip -4` (CGNAT 100.64.0.0/10 fallback) and binds the SSE listener there only; (3) spawns mcp-proxy with `--pass-environment -- <node> <self>` so a fresh stdio replicant becomes the backend. No bearer token in MVP — Tailscale's WireGuard tunnel is the trust boundary; `--host` overrides for users on other private networks. The stdio code path is unchanged.
Alternatives: Reimplement HTTP/SSE inside replicant-mcp (rejected: duplicates mcp-proxy, expands scope from "Android tools" to "MCP transport"); require `mcp-proxy` to be installed separately (rejected: violates the project's "Just Works UX" decision and adds a Python install step); Docker-only deployment recipe (rejected: USB-passthrough on macOS is fiddly and DECISIONS.md is npm-first); bearer-token auth from day one (rejected: redundant on Tailscale, can be added later if anyone asks). Per-client `ServerContext` isolation for concurrent agents was also rejected for MVP and is documented as a known limitation in `docs/remote.md`.
Refs: src/cli/serve.ts, src/services/tailscale.ts, docs/remote.md, https://github.com/sparfenyuk/mcp-proxy
