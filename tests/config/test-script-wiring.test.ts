...
describe("THE-100: npm test wires the contract guard locally", () => {
  ...
  it("`npm test` runs vitest one-shot and then check:contracts", () => {
    // Must one-shot (no watcher), and must invoke the contract check.
    expect(pkg.scripts.test).toMatch(/vitest --run/);
    expect(pkg.scripts.test).toMatch(/check:contracts/);
  });
